...
export default async function TransparencyPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <>
    <PageHero eyebrow={isKu ? "بەڕێوەبردن و متمانە" : "Governance & trust"} title={isKu ? "متمانە لە هەڵسوکەوتی ڕۆژانەوە دەست پێدەکات." : <>Trust begins with<br /><em>everyday conduct.</em></>} intro={<p>{isKu ? "زەنێ بەرپرسیارە لەبەردەم ئەو کەسانەی خزمەتیان دەکات، خێزان، هاوبەش و کۆمەڵگا." : "Zane is accountable to the people it serves, their families, its partners and the wider community."}</p>} aside={<p>{isKu ? "ڕێکاری گشتی بۆ پاراستن، زانیاری، سکاڵا و دەستگەیشتن لەم بەشەدا بەردەستن." : "Public routes for safeguarding, privacy, complaints and accessibility are available here."}</p>} />

    <section className="section-pad"><div className="shell"><SectionHeading eyebrow={isKu ? "پابەندبوونەکانمان" : "Our public commitments"} title={isKu ? "ستانداردەکان دەبێت لە کرداردا دیار بن." : "Standards should be visible in practice."} /><div className="trust-commitments">{commitments.map(([h,p],i) => <article key={h}><span>0{i+1}</span><h3>{h}</h3><p>{p}</p></article>)}</div></div></section>

    <section className="legal-profile-section section-pad section-ink"><div className="shell legal-profile-grid"><div><p className="eyebrow eyebrow--gold">{isKu ? "ناسنامەی دامەزراوە" : "Organizational identity"}</p><h2>{isKu ? "زانیاریی بنەڕەتی، لە یەک شوێندا." : "The essential public record, in one place."}</h2></div><dl>
