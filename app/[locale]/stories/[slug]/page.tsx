...
export default async function StoryPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  return <article className="article-page"><header className="article-header"><div className="shell article-header__inner"><Link className="article-back" href={`/${locale}/stories`}>← {isKu ? "هەواڵ و تێڕوانین" : "News & insight"}</Link><p className="eyebrow eyebrow--gold">{story.kind[locale]}</p><h1>{story.title[locale]}</h1><p className="article-dek">{story.excerpt[locale]}</p><div className="article-meta"><span>{story.readTime[locale]}</span><span>{story.date}</span></div></div></header><div className="shell article-layout"><aside><p>{isKu ? "لە زەنێ" : "From Zane"}</p><span>{isKu ? "بەشێک لە تۆماری گشتیی کار و فێربوونی ڕێکخراوەکە." : "Part of Zane’s public record of work and learning."}</span></aside><div className="article-body">{story.sections.map((section) => <section key={section.heading.en}><h2>{section.heading[locale]}</h2>{section.body[locale].map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</section>)}</div></div><footer className="article-footer"><div className="shell"><p>{isKu ? "دواتر چی بخوێنمەوە؟" : "Continue reading"}</p><Link href={`/${locale}/stories`}>{isKu ? "هەموو بابەتەکان" : "Explore all updates"} ↗</Link></div></footer></article>;
}
