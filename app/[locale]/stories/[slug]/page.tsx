...
export default async function StoryPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  return <article className="article-page"><header className="article-header"><div className="shell article-header__inner"><Link className="article-back" href={`/${locale}/stories`}>← {isKu ? "چیرۆک و فێربوون" : "Stories & learning"}</Link><p className="eyebrow eyebrow--gold">{story.kind[locale]}</p><h1>{story.title[locale]}</h1><p className="article-dek">{story.excerpt[locale]}</p><div className="article-meta"><span>{story.readTime[locale]}</span><span>{story.date}</span></div></div></header><div className="shell article-layout"><aside><p>{isKu ? "لە زەنێ" : "From Zane"}</p><span>{isKu ? "ئەم نووسراوە بەشێکە لە تۆماری گشتی فێربوون و دامەزراندنمان." : "Part of our public record of learning and institution-building."}</span></aside><div className="article-body">{story.sections.map((section) => <section key={section.heading.en}><h2>{section.heading[locale]}</h2>{section.body[locale].map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</section>)}</div></div><footer className="article-footer"><div className="shell"><p>{isKu ? "دواتر چی بخوێنمەوە؟" : "Continue reading"}</p><Link href={`/${locale}/stories`}>{isKu ? "هەموو تێبینییەکان ببینە" : "Explore all notes"} ↗</Link></div></footer></article>;
}
