...
export default async function GetInvolvedPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <>
    <PageHero
      ...
      title={isKu ? "بە کات و شارەزاییی خۆت بەشدار بە." : <>Contribute time.<br /><em>Apply expertise.</em></>}
      intro={<p>{isKu ? "تۆڕی خۆبەخشان و پسپۆڕانی زەنێ بۆ ئەرکی دیاریکراو، بە ماوە و سەرپەرشتیی دیاریکراو ڕێکدەخرێت." : "Zane’s volunteer and professional network is organized around defined roles, time commitments and supervision."}</p>}
    ...
    <section className="section-pad involvement-paths-section"><div className="shell"><SectionHeading eyebrow={isKu ? "ڕێگاکانی بەشداری" : "Ways to contribute"} title={isKu ? "ڕێگایەک هەڵبژێرە کە لەگەڵ کات و شارەزاییت دەگونجێت." : "Choose what fits your time and skills."} /><div className="involvement-paths">
      {[
        ...
        ["02", isKu ? "تۆڕی پسپۆڕان" : "Professional network", isKu ? "بە شارەزاییی خۆت لە تەندروستی، پەروەردە، یاسا، دارایی، زانیاری، پاراستن یان ڕاگەیاندن بەشدار بە." : "Contribute expertise in health, education, law, finance, data, safeguarding or communications."],
    ...
    <section className="form-section section-pad" id="volunteer"><div className="shell form-section__grid"><div className="form-section__intro"><p className="eyebrow">{isKu ? "تۆماری حەز" : "Volunteer interest"}</p><h2>{isKu ? "بواری بەشداریی خۆت دیاری بکە." : "Register your area of contribution."}</h2><p>{isKu ? "زانیاریی کورت دەربارەی کات و شارەزاییی خۆت بنێرە. ئەگەر ڕۆڵێک لەگەڵ زانیارییەکانت بگونجێت، زەنێ پەیوەندیت پێوە دەکات." : "Share your availability and relevant experience. Zane will contact you when a defined role matches your information."}</p><a className="text-link" href={`mailto:${site.volunteerEmail}`}>{site.volunteerEmail}<span>↗</span></a></div><OpportunityForm locale={locale} kind="volunteer" /></div></section>
