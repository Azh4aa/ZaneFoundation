...
export default async function PartnerPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <>
    ...
    <section className="partner-brief section-pad"><div className="shell partner-brief__grid"><div><p className="eyebrow">{isKu ? "یەکەم گفتوگۆ" : "The first conversation"}</p><h2>{isKu ? "پێمان بڵێ کێیت و چ گۆڕانێک گرنگە بۆت." : "Tell us who you are and what change matters to you."}</h2></div><div><p>{isKu ? "ناوی دامەزراوە، جۆری هاوبەشی، بوار یان پرۆگرامی جێی سەرنج، سنووری بودجە و خشتەی کاتی خۆتان لە ئیمەیڵێکدا بنێرن. ئێمە بە پاکێجی پەیوەندیدار و هەنگاوی داهاتوو وەڵام دەدەینەوە." : "Email your organization, partnership interest, program focus, indicative budget range and timeline. We will respond with the relevant pack and a proposed next step."}</p><a className="button button--dark" href={`mailto:${site.email}?subject=Partnership%20inquiry%20%E2%80%94%20Zane%20Foundation`}>{site.email}</a><Link className="text-link" href={`/${locale}/transparency`}>{isKu ? "پێش گفتوگۆ ڕوونکردنەوە ببینە" : "Review transparency first"}<span>↗</span></Link></div></div></section>
  </>;
}
